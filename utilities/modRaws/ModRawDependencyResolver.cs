...
// independent), saves, and publishes the result for ModSort to read. Deliberately not called from Initialize()/ReloadModpacksFromDisk() — this
// is invoked explicitly from exactly three UI-level call sites (startup, the manual reload button, ModSort) so it doesn't run on every automatic
